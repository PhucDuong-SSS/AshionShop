...
class cart
{
     ...
     */
    public function add_to_cart($id, $quantity,$customer_id)
    {
        ...
        if ($result_check) {
            while ($check_cart = $result_check->fetch_assoc()) {
                $newquantity = $check_cart['quantity'];
                if ($result['product_remain'] > $newquantity) {
                    $newquantity+=$quantity;
                    $update_quantity = "UPDATE tbl_cart SET quantity='$newquantity' WHERE productId = '$id' AND sId = '$sId'";

                    $result_update_check = $this->db->update($update_quantity);
                }
            }
        } else {
            ...
            if ($result['product_remain'] > $quantity) {
                ...
                $query_insert = "INSERT INTO tbl_cart(productId,customerId,productName,quantity,sId,price,image) VALUES('$id','$customer_id','$productName','$quantity','$sId','$price','$image' )";
    ...
    {  
        $sId = session_id();
        $query = "SELECT * FROM tbl_cart WHERE sId = '$sId' AND customerId='$customer_id'";
        $result = $this->db->select($query);
        
        
        return $result;
    }
    public function update_quantity_Cart($proId, $cartId, $quantity,$customer_id)
    {
    ...
    public function del_product_cart($cartid,$customer_id)
    {
        $cartid = mysqli_real_escape_string($this->db->link, $cartid);
        $query = "DELETE FROM tbl_cart WHERE cartId = '$cartid' AND customerId ='$customer_id'";
     ...
     * 
     */
    public function showQuantity($customerId)
    {   $quantity= 0;
        $sId = session_id();
    ...
    {
        $sId = session_id();
        $query = "DELETE FROM tbl_cart WHERE sId = '$sId' AND customerId = '$customer_id' ";
        $result = $this->db->select($query);
        return $result;
    }
    /**
    ...
    /**
     * insert order by customer_id
     */
    public function insertOrder($customer_id,$customer_note)
    {
        $sId = session_id();
        $note = mysqli_real_escape_string($this->db->link, $customer_note);
        $query = "SELECT * FROM tbl_cart WHERE sId = '$sId' AND customerId = '$customer_id'";
